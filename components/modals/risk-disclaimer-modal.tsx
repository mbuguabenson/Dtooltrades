...
export function RiskDisclaimerModal({ isOpen, onClose, onAccept, theme = "dark" }: RiskDisclaimerModalProps) {
    return (
        <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
            <DialogContent className={`sm:max-w-[500px] border-none p-0 overflow-hidden rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.5)] ${theme === "dark" ? "bg-[#0a0f1e] text-white" : "bg-white text-slate-900"
                }`}>
                <div className="relative p-8">
                    ...
                    <DialogHeader className="relative z-10 flex flex-col items-center text-center space-y-4">
                        <div className={`w-16 h-16 rounded-2xl flex items-center justify-center shadow-lg ${theme === "dark"
                                ? "bg-amber-500/10 border border-amber-500/20 text-amber-500"
                                : "bg-amber-50 border border-amber-200 text-amber-600"
                            }`}>
                            <AlertTriangle className="h-8 w-8" />
                        </div>
                        <DialogTitle className="text-2xl font-bold tracking-tight">Important Risk Disclosure</DialogTitle>
                    </DialogHeader>

                    <div className={`mt-6 space-y-4 relative z-10 text-sm leading-relaxed ${theme === "dark" ? "text-slate-300" : "text-slate-600"
                        }`}>
                        <p className="font-medium text-amber-500 uppercase tracking-widest text-[10px]">Please read carefully</p>

                        <p className="bg-white/5 p-4 rounded-2xl border border-white/5 backdrop-blur-sm">
                        ...
                        <div className="space-y-3">
                            <p>Please understand the following risks before trading Deriv products:</p>
                        ...
                        <p className={`p-4 rounded-2xl font-semibold border ${theme === "dark"
                    ...
                    <DialogFooter className="mt-8 flex flex-col sm:flex-row gap-3 relative z-10">
                        <Button
                            onClick={onAccept}
                            className={`w-full h-14 rounded-2xl text-base font-bold transition-all duration-300 flex items-center justify-center gap-2 group ${theme === "dark"
